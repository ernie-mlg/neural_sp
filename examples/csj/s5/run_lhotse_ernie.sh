...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ] && [ ! -e ${data}/.done_stage_0_${datasize} ]; then
    ...
    if [ ! -e data/csj-data/.done_make_all ]; then
        ...
        # Make data/csj_data
        erpkg/data_loader/ernie_autorun.sh $data_set data/csj-data $morph_flag
    fi
    wait
    ...
    erpkg/local/csj_data_prep.sh data/csj-data
    for x in eval1 eval2 eval3; do
        #local/csj_eval_data_prep.sh ${data}/csj-data/eval ${data} ${x} || exit 1;
	erpkg/local/csj_eval_data_prep.sh ${data}/csj-data/eval ${x} || exit 1;
