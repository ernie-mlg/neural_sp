...
if [ -f $tdir/$eval_num/wavflist.scp ];then
  rm $tdir/$eval_num/wavflist.scp
fi
wav_list=$(ls $tdir/$eval_num | sort)
for id in $wav_list;do
  wav_path=$(cat $tdir/$eval_num/$id/*-wav.list)
  echo $id $wav_path >> $tdir/$eval_num/wavflist.scp
done
cat $tdir/$eval_num/wavflist.scp > $dir/wavflist.scp
