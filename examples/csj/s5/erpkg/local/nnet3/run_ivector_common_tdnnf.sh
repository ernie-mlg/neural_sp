...
train_set=train_nodup   # you might set this to e.g. train.
...
if [ $stage -le 2 ] && [ -f data/${train_set}_sp_hires/feats.scp ]; then
  ...
  rm  data/${train_set}_sp_hires/feats.scp
  #exit 1
fi
...
if [ $stage -le 2 ]; then
  ...
  for datadir in ${train_set}_sp eval1 eval2 eval3; do
...
if [ -f data/${train_set}_sp/feats.scp ] && [ $stage -le 9 ]; then
  ...
  rm data/${train_set}_sp/feats.scp
  #exit 1;
fi
