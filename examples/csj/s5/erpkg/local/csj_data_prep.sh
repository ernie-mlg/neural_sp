...
then 
  ...
  ##cat $CSJ/*/{A,M}*/*-wav.list 2>/dev/null | sort > $dir/wav.flist # Using "Academic lecture" and "other" data
  #cat $CSJ/*/Av*/*-wav.list 2>/dev/null | sort >> $dir/wav.flist
  #cat $CSJ/*/A{5..9}*/*-wav.list 2>/dev/null | sort >> $dir/wav.flist
  #cat $CSJ/*/E*/*-wav.list 2>/dev/null sort >> $dir/wav.flist
  #cat $CSJ/*/As*/*-wav.list 2>/dev/null | sort >> $dir/wav.flist
  #cat $CSJ/core/A{0,1}*/*-wav.list 2>/dev/null | sort >> $dir/wav.flist
  #cat $CSJ/noncore/{A,M}*/*-wav.list 2>/dev/null | sort >> $dir/wav.flist
  #cat $CSJ/*/*/*-wav.list 2>/dev/null | sort > $dir/wav.flist
  #echo "Making wav.flist is skipped, because it was already done."
  #disc='core noncore'
  disc='core'
  for vol in $disc;do
    if [ -f $CSJ/$vol/wavflist.scp ];then
      rm $CSJ/$vol/wavflist.scp
    fi
    wav_list=$(ls $CSJ/$vol | sort)
    for id in $wav_list;do
      wav_path=$(cat $CSJ/$vol/$id/*-wav.list)
      echo $id $wav_path >> $CSJ/$vol/wavflist.scp
    done
    cat $CSJ/$vol/wavflist.scp >> $dir/wavflist.scp
  done
fi
...
#export LC_ALL=C
#sort -k2 $dir/utt2spk > $dir/utt2spk_tmp
#cp $dir/utt2spk_tmp $dir/utt2spk

sort -k2 $dir/utt2spk | utils/utt2spk_to_spk2utt.pl > $dir/spk2utt || exit 1;
